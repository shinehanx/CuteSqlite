...
public:
	...
	static void parseSectionSettings(std::wstring & iniProfilePath, std::wstring &section, Setting &settings)
	{
		...
		wchar_t buff[15360] = {0}; // 返回的数据中，以'\0'结束一个section; 所有的section字符串结束后，最后再加一个\0;
		memset(buff, 0, sizeof(buff));
		DWORD buff_len = GetPrivateProfileSectionW(section.c_str(), buff,  15360, iniProfilePath.c_str());
		if (buff_len == 0) {
			return ;
		}
