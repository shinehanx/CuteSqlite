 ...
 * @file   TriggerDialog.h
 * @brief  Create or alter a trigger for database 
 * 
 * @author Xuehan Qin
 * @date   2023-10-24
 *********************************************************************/
...
class TriggerDialog : public QDialog<TriggerDialog>
{
public:
	BEGIN_MSG_MAP_EX(TriggerDialog)
		MESSAGE_HANDLER(WM_INITDIALOG, OnInitDialog)
		MESSAGE_HANDLER(WM_DESTROY, OnDestroy)
		MESSAGE_HANDLER(WM_SHOWWINDOW, OnShowWindow)
		CHAIN_MSG_MAP(QDialog<TriggerDialog>)
		REFLECT_NOTIFICATIONS()
	END_MSG_MAP()
	TriggerDialog(HWND parentHwnd);

private:
	bool isNeedReload = true;
	HWND parentHwnd = nullptr;

	CStatic triggerNameLabel;
	CEdit triggerNameEdit;
	...
	void createTriggerNameElems(CRect & clientRect);
