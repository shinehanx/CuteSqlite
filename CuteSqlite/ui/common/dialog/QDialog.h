...
protected:
	...
	void createOrShowYesButton(QImageButton & win, CRect &clientRect);	
	void createOrShowNoButton(QImageButton & win, CRect &clientRect);
	...
	//用户定义
	virtual LRESULT OnClickYesButton(UINT uNotifyCode, int nID, HWND hwnd);
	virtual LRESULT OnClickNoButton(UINT uNotifyCode, int nID, HWND hwnd);
};
...
template <class T>
void QDialog<T>::createOrShowYesButton(QImageButton & win, CRect &clientRect)
{
	...
	if (::IsWindow(m_hWnd) && !win.IsWindow()) {
		win.Create(m_hWnd, rect, S(L"ok").c_str(), WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN  | BS_OWNERDRAW , 0, Config::QDIALOG_YES_BUTTON_ID);
...
template <class T>
void QDialog<T>::createOrShowNoButton(QImageButton & win, CRect &clientRect)
{
	CRect rect(clientRect.right - QDIALOG_BUTTON_WIDTH - 10, 
		clientRect.bottom - QDIALOG_BUTTON_HEIGHT - 10, 
		clientRect.right - 10, clientRect.bottom - 10);
	if (::IsWindow(m_hWnd) && !win.IsWindow()) {
		win.Create(m_hWnd, rect, S(L"cancel").c_str(), WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN  | BS_OWNERDRAW , 0, Config::QDIALOG_NO_BUTTON_ID);		
...
{
	if (::IsWindow(m_hWnd) && !win.IsWindow()) {
		DWORD dwStyle = WS_CHILD | WS_VISIBLE  | BS_RADIOBUTTON ;
		if (exStyle) dwStyle = dwStyle | exStyle;
		win.Create(m_hWnd, rect, text.c_str(), dwStyle , 0, id);
...
template <class T>
LRESULT QDialog<T>::OnClickYesButton(UINT uNotifyCode, int nID, HWND hwnd)
{
	EndDialog(nID);
	return 0;
}
...
template <class T>
LRESULT QDialog<T>::OnClickNoButton(UINT uNotifyCode, int nID, HWND hwnd)
{
	EndDialog(nID);
	return 0;
}
